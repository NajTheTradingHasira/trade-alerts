#!/usr/bin/env python3
"""
postmarket_screener_polygon.py — Polygon.io Post-Market Movers
═════════════════════════════════════════════════════════════════
Fetches ALL US stock snapshots in a single Polygon API call (~9,000 tickers),
filters for after-hours movers, posts to Slack, and saves CSV.

Uses: GET /v2/snapshot/locale/us/markets/stocks/tickers

Route keys: postmarket_winner, postmarket_loser

Usage:
  python postmarket_screener_polygon.py
  python postmarket_screener_polygon.py --min-change 5 --min-volume 100000
  python postmarket_screener_polygon.py --slack --config channel_config.json
  python postmarket_screener_polygon.py --dry-run
...
def filter_movers(snapshots: list[dict], min_change: float, min_volume: int,
    ...
    """
    Filter snapshots for after-hours movers.
...
def print_table(winners: list[dict], losers: list[dict]):
    ...
    if losers:
        header = f"  {'Ticker':<7} {'Price':>8} {'Chg %':>8} {'RVol':>6} {'Volume':>10} {'PrevCl':>8}"
        print(f"  📉 LOSERS (After-Hours Down)")
        print(f"  {'─' * 55}")
        print(header)
        print(f"  {'─' * 55}")
        for l in losers[:30]:
...
def write_csv(winners: list[dict], losers: list[dict]) -> str:
    ...
    filename = out_dir / f"postmarket_movers_{ts}.csv"
...
def build_slack_message(winners: list[dict], losers: list[dict]) -> str:
    ...
    now = datetime.now().strftime("%Y-%m-%d %H:%M ET")
    lines = [
        f"*:crescent_moon: APEX Post-Market Movers* — {now}",
        f"{len(winners)} winner(s) | {len(losers)} loser(s)",
        "",
    ]

    if winners:
        lines.append("*:chart_with_upwards_trend: WINNERS (After-Hours Up)*")
        for w in winners[:25]:
            lines.append(
    ...
    if losers:
        lines.append("*:chart_with_downwards_trend: LOSERS (After-Hours Down)*")
        for l in losers[:25]:
            lines.append(
    ...
    if not winners and not losers:
        lines.append("_No significant post-market movers today._")
...
def send_to_slack(winners: list[dict], losers: list[dict], config_path: str):
    ...
    if winners:
        router.send("postmarket_winner", msg)
        print(f"  ✓ Posted winners to #postmarket-movers + #firehose")
    if losers and not winners:
        router.send("postmarket_loser", msg)
        print(f"  ✓ Posted losers to #postmarket-movers + #firehose")
    if not winners and not losers:
        router.send("postmarket_winner", msg)
        print(f"  ✓ Posted 'no movers' to #postmarket-movers + #firehose")
...
def _send_direct(winners: list[dict], losers: list[dict]):
    ...
    for channel_id, name in [("C0APMTV3BRA", "postmarket-movers"), ("C0ANZDPD4AU", "firehose")]:
        try:
...
# ═══════════════════════════════════════════════════════════════
# Callable entry point for daily_scan.py
# ═══════════════════════════════════════════════════════════════

def run_postmarket_screen(min_change: float = 3.0, min_volume: int = 50_000,
                          min_price: float = 5.0) -> tuple[list[dict], list[dict]]:
    """
    Entry point for daily_scan.py integration.
    Returns (winners, losers) as lists of dicts.
    """
    ...
    winners, losers = filter_movers(snapshots, min_change, min_volume, min_price)
...
def main():
    parser = argparse.ArgumentParser(description="Polygon.io Post-Market Movers Screener")
    ...
    print(f"\n{'=' * 60}")
    print(f"  APEX Post-Market Screener (Polygon.io)")
    ...
    if args.dry_run:
        print("  [DRY RUN] Would fetch Polygon snapshots and filter for post-market movers")
